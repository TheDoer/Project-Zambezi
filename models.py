...
class Catalogue(BaseModel):
	long_code = CharField(unique=True)
	short_code = CharField(null=True, unique=True)
	available = BooleanField(default=False)

	@classmethod
	def create_entry(cls, long_code, short_code, available):
		return cls.create(long_code=long_code, short_code=short_code, available=available)

class User(BaseModel, UserMixin):
	username = CharField(default="unknown")
	...
	@classmethod
	def create_user(cls, username='sales', phone=775527640, email='sales@roundshopper.com', account_type='supplier',
		password='Tatendaz01',	address='77 Quorn Avenue, Mount Pleasant, Harare, Zimbabwe', profile='default.png'):

		cls.create(username=username, phone=phone, email=email, profile=profile, account_type=account_type,
			password=generate_password_hash(password, 12), address=address)
...
class Stock(BaseModel):
	...
	quantity = CharField(max_length=20) #this is the quantity of units above. e.g 9kg 9 is the quantity, kg is the units
	...
	@classmethod
	def enter_stock(cls, product, first_description, unit, brand, supplier, price, moq,
		minimum_quantity=minimum_quantity, quantity=quantity,
		second_description=None, third_description=None, image='default.png'):
		...
		cls.create(product=product, first_description=first_description,
			...
			supplier=supplier, price=price, moq=moq, image=image)
...
class Order(BaseModel):
	...
	date_fulfilled = DateTimeField(default=None, null=True)
	date_cancelled = DateTimeField(default=None, null=True)

	@classmethod
	def make_order(cls, buyer, stock, quantity, price):

		return cls.create(buyer=buyer, stock=stock, quantity=quantity, price=price)

class DeletedOrder(BaseModel):
	buyer = ForeignKeyField(User, related_name='deleted_orders')
	stock = ForeignKeyField(Stock, related_name='deleted_orders')
	quantity = IntegerField()
	price = DoubleField()
	payment = DoubleField(default=0)
	ready = BooleanField(default=False)
...
if __name__ == '__main__':
	DB.create_tables([User, Product, Unit, Descriptor, Brand, Stock, Order, Suggestion, Catalogue, DeletedOrder], safe=True)
